...
- (void)viewDidLoad
{
    ...
    GEGifView* gifView1 = [[GEGifView alloc] initWithFrame:CGRectMake(0, 0+20, 160, 160)];
    gifView1.backgroundColor = [UIColor grayColor];
    [self.view addSubview:gifView1];
    gifView1.fileName = @"cat_justAImage.gif"; // one picture named *.gif
    [gifView1 start];
    
    GEGifView* gifView2 = [[GEGifView alloc] initWithFrame:CGRectMake(160, 0+20, 160, 160)];
    gifView2.backgroundColor = [UIColor magentaColor];
    [self.view addSubview:gifView2];
    gifView2.fileName = @"heart_oneFrame.gif"; // gif which has one frame
    [gifView2 start];
    
    GEGifView* gifView3 = [[GEGifView alloc] initWithFrame:CGRectMake(0, 160+20, 320, 320)];
    gifView3.backgroundColor = [UIColor brownColor];
    [self.view addSubview:gifView3];
    gifView3.fileName = @"cat_jiaFei.gif"; // gif which has many frames
    [gifView3 start];
}
